...
use crate::utils::{get_file_name, read_file_to_buffer, validate_zip_archive};
...
impl TestopsApi {
    ...
    pub async fn get_launch_by_id(
        &self,
        launch_id: u32,
    ) -> Result<GetLaunchByIdResponse, ApiError> {
        self.client
            .get::<GetLaunchByIdResponse, ()>(&format!("{}/launch/{}", self.api_prefix, launch_id))
            .await
    }

    pub async fn post_upload_report(
        &self,
        file_path: &Path,
        launch_info: &LaunchInfo,
    ) -> Result<ResponseLaunchUpload, ApiError> {
        // Читаем в буфер файл
        ...
        self.client
            .post_multipart_file::<ResponseLaunchUpload, ()>(
                &format!("{}/launch/upload", self.api_prefix),
                form,
            )
            .await
    }
    ...
    pub async fn get_all_project_ids(&self) -> Result<HashSet<u32>, ApiError> {
        ...
        loop {
            let response = self
                .client
                .get::<ResponseGetAllProject, ()>(&format!(
                    "{}/project?page={}",
                    self.api_prefix, current_page
                ))
                .await?;
            project_ids.extend(response.content.iter().map(|project_info| project_info.id));
    ...
    pub async fn get_project_info_by_id(&self, project_id: &u32) -> Result<ProjectInfo, ApiError> {
        self.client
            .get::<ProjectInfo, ()>(&format!("{}/project/{}", self.api_prefix, project_id))
            .await
    }
    ...
    pub async fn get_test_case_overview_by_id(
        &self,
        test_case_id: &u32,
    ) -> Result<TestCaseOverview, ApiError> {
        self.client
            .get::<TestCaseOverview, ()>(&format!(
                "{}/testcase/{}/overview",
                self.api_prefix, test_case_id
            ))
            .await
    }
...
mod tests {
    ...
    impl TestopsApi {
        ...
        pub async fn mock_post_upload_report(
            ...
            server_mock
                .mock("POST", "/api/rs/launch/upload")
                .match_header(
                    "content-type",
                    mockito::Matcher::Regex(r"multipart/form-data; boundary=.*".into()),
                )
                .match_body(mockito::Matcher::Regex(
                    "Content-Disposition: form-data; name=\"info\"".into(),
                ))
                .match_body(mockito::Matcher::Regex(
                    "Content-Disposition: form-data; name=\"archive\"".into(),
                ))
                .with_status(200)
        ...
        pub async fn mock_get_project_by_id(
            server_mock: &mut mockito::ServerGuard,
            mock_response: &ProjectInfo,
        ) {
        ...
        pub async fn mock_get_all_projects(server_mock: &mut mockito::ServerGuard) {
            ...
            let response_page_1 = ResponseGetAllProject {
                total_pages: 2,
                content: vec![ProjectInfo::new(3, "Project3")],
            };
            server_mock
                .mock("GET", "/api/rs/project?page=0")
                .with_status(200)
                .with_header("content-type", "application/json")
            ...
            server_mock
                .mock("GET", "/api/rs/project?page=1")
                .with_status(200)
                .with_header("content-type", "application/json")
        ...
        pub async fn mock_get_test_case_overview_by_id(
            ...
            let endpoint =
                mockito::Matcher::Exact(format!("/api/rs/testcase/{}/overview", &mock_response.id));
            server_mock
                .mock("GET", endpoint)
                .with_status(200)
                .with_header("content-type", "application/json")
        ...
        pub async fn mock_get_test_case_scenario(
            ...
            let endpoint =
                mockito::Matcher::Exact(format!("/api/rs/testcase/{}/step", test_case_id));
            server_mock
                .mock("GET", endpoint)
                .with_status(200)
                .with_header("content-type", "application/json")
    ...
    async fn test_post_upload_report_mock() {
        ...
        let result = testops_api
            .post_upload_report(&path_to_report, &LaunchInfo::default())
            .await;
        assert!(result.is_ok());
